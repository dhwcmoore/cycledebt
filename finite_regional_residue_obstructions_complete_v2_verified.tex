...
\begin{remark}[Computational criterion]
Cycle-faithfulness is a linear-algebra condition over \(\Q\). It holds precisely
when the homogeneous system
\[
\begin{pmatrix}\partial' & 0 \\ \rho_* & -z\end{pmatrix}
\begin{pmatrix}z'\\\lambda\end{pmatrix}=0
\]
has a solution with \(\lambda\neq 0\). When no such solution exists, the system
forces \(\lambda=0\), and cycle-faithfulness is algebraically impossible.
\end{remark}

\begin{theorem}[Cycle-lift persistence]\label{thm:cycle-lift}
Let \(N\) be a finite oriented nerve and let
\(r\in C^1(N;\Q)\) be a cocycle. Suppose there exists a cycle
\(z\in Z_1(N;\Q)\) such that
\[
\langle z,r\rangle\neq 0.
\]
...
\[
\langle z',\rho^*\alpha\rangle
=
\langle \rho_*z',\alpha\rangle
\]
for all \(z'\in C_1(N';\Q)\) and all \(\alpha\in C^1(N;\Q)\).

If there exists a refined cycle \(z'\in Z_1(N';\Q)\) and a nonzero rational
\(\lambda\in\Q^\times\) such that
\[
\rho_*z'=\lambda z,
\]
then
...
\emph{Direct obstruction persistence.} There exists a refined cycle
\(z'\in Z_1(N';\Q)\) with \(\langle z',\rho^*r\rangle\neq 0\).
All four declared refinements satisfy this.
Proved by Lemma \ref{lem:cycle-pairing} in \(N'\).

\emph{Witness persistence (cycle-faithfulness).} The original
obstruction cycle \(z\) admits a nonzero rational lift via
\(\rho_*\). Two of the four declared refinements satisfy this.
When it holds, Theorem \ref{thm:cycle-lift} applies and the
persistence proof requires no new cycle computation.

The first can hold even when the second fails.
\end{remark}

\begin{conjecture}[Balanced loop refinement]
...
The two cycle-faithful refinements in Section \ref{sec:invariance-refinement}
support this conjecture. The insert-bridge refinement replaces the edge
\(U_1U_2\) with a path through a new vertex, leaving all original vertices
on \(z\) unchanged in degree. The subdivide-all refinement eliminates the
original vertices entirely, replacing each with a pair. In both cases,
flow conservation at original vertices creates no obstruction to the lift.
The two non-cycle-faithful refinements each leave an original vertex
(\(U_2\) or \(U_1\)/\(U_3\)) with new incident edges, and this is
the precise source of the algebraic impossibility.
\end{remark}
