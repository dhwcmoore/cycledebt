...
\begin{theorem}[Persistence under admissible refinement]\label{thm:admissible-refinement}
...
Subdivide all regions & Yes & \(1/4\) & Theorem \ref{thm:admissible-refinement} \\
Insert bridge \(U_1\)--\(U_2\) & Yes & \(1\) & Theorem \ref{thm:admissible-refinement} \\
\bottomrule
\end{tabular}
\end{center}
...
\ref{thm:admissible-refinement}.

The distinction is verified computationally in
\texttt{admissible\_refinement\_theorem.py}.
\end{remark}

\begin{remark}[What remains for universality]
A universal result would require identifying all refinements that admit a
nonzero cycle-lift and proving the lift exists in each case.
The present paper does not claim that theorem.
It identifies it as the next mathematical target.
\end{remark}
