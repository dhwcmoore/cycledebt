...
\begin{conjecture}[Balanced cycle-fibre]\label{conj:cycle-fibre}
Let \(z\) be a simple rational cycle in a finite oriented nerve \(N\).
Let \(\rho:N'\to N\) be a refinement equipped with a chain pushforward
\(\rho_*\). Suppose the full preimage of the support of \(z\) contains
a divergence-free rational circulation whose pushforward crosses every
edge of \(z\) with the same nonzero rational degree. Then \(\rho\) is
cycle-faithful relative to \(z\).
\end{conjecture}

\begin{remark}[Evidence and rank interpretation]
For the insert-bridge and subdivide-all refinements, such a uniform
circulation exists: the bridge cycle traverses both halves of the split
edge with the same weight, and the diagonal path through the fully
subdivided complex crosses each cluster with weight \(1/4\). In both
cases \(\operatorname{rank}(PK)=\operatorname{rank}([PK\;\;z])=1\).

For the single-region subdivisions, no uniform circulation exists.
The generator of \(\operatorname{im}(P|_{Z_1(N')})\) has a non-uniform
ratio relative to \(z\). For subdivide-\(U_1\):
\[
\text{generator} = (-\tfrac{1}{2},-1,-1,\tfrac{1}{2}),
\quad
\frac{\text{gen}[e]}{z[e]} =
\begin{cases}\tfrac{1}{2} & e\in\{U_1U_2,U_1U_4\}\\1 & e\in\{U_2U_3,U_3U_4\}\end{cases}.
\]
The split edges (those incident to the subdivided \(U_1\)) contribute
ratio \(\tfrac{1}{2}\); the unchanged edges contribute ratio \(1\).
No scalar multiple of this generator equals \(z\), so
\(\operatorname{rank}([PK\;\;z])=2\) and \(z\notin\operatorname{im}(PK)\).

Proving Conjecture \ref{conj:cycle-fibre} would give a geometric
sufficient condition for cycle-faithfulness. The two theorems below
