...
For the single-region subdivisions, no uniform circulation exists: the
junction vertex forces an imbalance between the split-edge cluster and
the unchanged adjacent edge, giving \(\operatorname{rank}(PK)=1\) but
\(\operatorname{rank}([PK\;\;z])=2\). This confirms that the image of
\(Z_1(N')\) under \(\rho_*\) is a one-dimensional subspace of
\(C_1(N;\Q)\) that does not contain \(z\).

Proving Conjecture \ref{conj:cycle-fibre} would give a geometric
sufficient condition for cycle-faithfulness and turn the current
classification into a theorem about an explicit refinement class.
\end{remark}
